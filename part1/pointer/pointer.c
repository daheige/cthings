...
// 指针是一种保存变量地址的变量，这个变量是一个0x开头的16进制的数字
// 指针用途非常广泛，原因之一，指针通常是表达某个计算的唯一途径
// 另一个原因是，同其他方法比较起来，使用指针通常可以生成更高效、更紧凑的代码
// 谨慎地使用指针，便可以利用它写出简单、清晰的程序

// ANSI C一个重要的编号，它明确地制定来操作指针的规则。
// 另外，它使用void* 指向void的指针，来代替char *作为通用指针的类型。

// int *p 这里的声明，*ip的结果是int类型，p是指向int类型变量的指针。
// *ip的*运算符是间接寻址或间接引用运算符。当它作用于指针时候，将访问指针所指向的对象。
// 指针只能指向某种特定类型的对象，也就是说，每个指针都必须指向某种特定的数据类型。
// 一个例外是，指向void类型的指针，可以存放指向任何类型的指针，但它不能间接引用其自身。
